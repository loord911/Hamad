...
def build_agents(x):
    r=x; specs=[]
    def add(group,name,score,reason):
        # بعض عمليات NumPy مثل np.where تُرجع ndarray بدل Series.
        # نحولها إلى Series بنفس فهرس بيانات السعر حتى تعمل replace/shift
        # والمحاذاة في التحقق التاريخي بشكل صحيح.
        if not isinstance(score, pd.Series):
            score = pd.Series(score, index=r.index)
        specs.append({"Group":group,"Agent":name,"Score":clamp_series(score),"Reason":reason})
